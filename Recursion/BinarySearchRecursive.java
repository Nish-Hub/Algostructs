...
first index and rest of the array is taken care of by the recursion
...
if(k>arr[mid]) // shift right
    {
      return findingIdxH(arr,k,mid+1,h);  
      /*
      Recursive conversation here says 
  	 "I trust this helper will search it in the left part , i dont care how but i will get an index or -1" 
     */
      //l=mid+1;
  		//findingIdxIterative(arr,k,mid+1,h);  
      /*
      This function gives an int return which must be returned as an index . 
      so you can either save this in index or return this value
      */
  		//return findingIdxIterative(arr,k,mid+1,h)
    }
    else if(k<arr[mid]) // shift left
    {
       return  findingIdxH(arr,k,l,mid-1);
      //h=mid-1;
     // return  findingIdxIterative(arr,k,l,mid-1);
 
    }
    else   // when items match
      return mid;
  
//   return  -1;  // unreachable code here
}
int findingIdxH(int arr[],int k,int l,int h)
{
  int mid=(l+h)/2;
